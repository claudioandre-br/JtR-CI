...
curl -L --output /tmp/macOS-Intel_1_JtR.zip \
  ...
  https://api.github.com/repos/claudioandre-br/john-packages/actions/artifacts/2695700585/zip

curl -L --output /tmp/macOS-Intel_2_buildlog.zip \
  -H "Accept: application/vnd.github+json" \
  -H "Authorization: Bearer $GH_PERSONAL_TOKEN" \
  -H "X-GitHub-Api-Version: 2022-11-28" \
  https://api.github.com/repos/claudioandre-br/john-packages/actions/runs/13675235235/logs
...
echo "Mac Intel is at: $MACOSIntel_TEXT" >> $LOG_FILE
