...
if [[ "$TEST" == *"usual;"* ]]; then
    ...
    # Run docker
    docker run --cap-add SYS_PTRACE -v "$HOME":/root -v "$(pwd)":/cwd ghcr.io/openwall/john-ci:ubuntu.rolling sh -c "$docker_command"
    ...
    # Run docker
    docker run --cap-add SYS_PTRACE --privileged -v "$HOME":/root -v "$(pwd)":/cwd ghcr.io/openwall/john-ci:ubuntu.devel sh -c "$docker_command"
    ...
    # Run docker
    docker run --cap-add SYS_PTRACE -v "$HOME":/root -v "$(pwd)":/cwd ghcr.io/openwall/john-ci:ubuntu.opencl sh -c "$docker_command"
    ...
    # Run docker
    docker run -v "$HOME":/root -v "$(pwd)":/cwd ghcr.io/openwall/john-ci:centos.7 sh -c "$docker_command"
